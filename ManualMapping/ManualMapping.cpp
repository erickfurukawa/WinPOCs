...
int main(int argc, char** argv)
{
    ...
    if (GetPEHeaders(reinterpret_cast<char*>(buffer), &headers))
    {
        ...
        HANDLE hProc = OpenProcess(PROCESS_CREATE_THREAD | PROCESS_VM_READ | PROCESS_VM_WRITE | PROCESS_VM_OPERATION, false, pid);
        if (!hProc)
        {
        ...
        BYTE* baseAddr = reinterpret_cast<BYTE*>(VirtualAllocEx(hProc, reinterpret_cast<void*>(headers.pOptionalHeader->ImageBase), headers.pOptionalHeader->SizeOfImage, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE));
        if (!baseAddr)
        {
            baseAddr = reinterpret_cast<BYTE*>(VirtualAllocEx(hProc, nullptr, headers.pOptionalHeader->SizeOfImage, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE));
            if (!baseAddr)
            {
        ...
        if (!WriteProcessMemory(hProc, baseAddr, buffer, headers.pOptionalHeader->SizeOfHeaders, nullptr))
        {
        ...
        for (int i = 0; i < headers.pFileHeader->NumberOfSections; ++pSectionHeader, i++)
        {
            if (pSectionHeader->SizeOfRawData)
            {
                ...
                if (!WriteProcessMemory(hProc, baseAddr + pSectionHeader->VirtualAddress,
                    buffer + pSectionHeader->PointerToRawData, pSectionHeader->SizeOfRawData, nullptr))
                {
        ...
        BYTE* dataAddr = reinterpret_cast<BYTE*>(VirtualAllocEx(hProc, nullptr, sizeof(ManualMappingData), MEM_RESERVE | MEM_COMMIT, PAGE_EXECUTE_READWRITE));
        if (!dataAddr)
        {
        ...
        WriteProcessMemory(hProc, dataAddr, &data, sizeof(ManualMappingData), nullptr);
        ...
        BYTE* shellcodeAddr = reinterpret_cast<BYTE*>(VirtualAllocEx(hProc, nullptr, 0x1000, MEM_RESERVE | MEM_COMMIT, PAGE_EXECUTE_READWRITE));
        if (!shellcodeAddr)
        {
        ...
        WriteProcessMemory(hProc, shellcodeAddr, Shellcode, 0x1000, nullptr);
        ...
        HANDLE hThread = CreateRemoteThread(hProc, nullptr, 0, reinterpret_cast<LPTHREAD_START_ROUTINE>(shellcodeAddr), dataAddr, 0, nullptr);
        if (!hThread)
        {
            std::cerr << "Could not create a thread.\n";
            VirtualFreeEx(hProc, baseAddr, 0, MEM_RELEASE);
            VirtualFreeEx(hProc, dataAddr, 0, MEM_RELEASE);
            VirtualFreeEx(hProc, shellcodeAddr, 0, MEM_RELEASE);
            return 1;
        }
        CloseHandle(hThread);
        ...
        while (status != 1)
        {
            ...
            ReadProcessMemory(hProc, dataAddr, &dataCheck, sizeof(ManualMappingData), nullptr);
